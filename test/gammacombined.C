TH1F* histbkg(string cuts="",string var="mjj"){
    string ges="/afs/cern.ch/work/n/nsolomon/FCC/WorkDir/OutDir/Bkg/heppy.analyzers.examples.zh_had.TreeProducer.TreeProducer_1/tree.root";
    TFile *file=new TFile(ges.c_str());
    TTree *eventsWW;
    file->GetObject("events", eventsWW);
    eventsWW->Draw((var+">>+htemp(13,112,138)").c_str(),cuts.c_str());
...
TH1F* histsig(string cuts="",string var="mjj"){
    string ges="/afs/cern.ch/work/n/nsolomon/FCC/WorkDir/OutDir/ZH/heppy.analyzers.examples.zh_had.TreeProducer.TreeProducer_1/tree.root";
    TFile *file=new TFile(ges.c_str());
    TTree *eventsWW;
    file->GetObject("events", eventsWW);
    eventsWW->Draw((var+">>+htemp(13,112,138)").c_str(),cuts.c_str());
...
THStack *hs = new THStack("hs","Di-photon invariant mass;Higgs mass (GeV);Events/2Gev");
...
hs->Add(h2);
hs->Add(h1);
TH1F *h3=new TH1F("h3","Higgs mass",13,112,138);
h3->Add(h1);
h3->Add(h2);

TF1* fit1 = new TF1("fit1", "gaus", 112, 138);
h1->Fit("fit1");
Double_t integral1 = fit1->Integral(112, 138);
...
TF1* fit2 = new TF1("fit2", "pol3", 112, 138);
h2->Fit("fit2");
Double_t integral2 = fit2->Integral(112, 138);
